...
namespace ResumeApp
{
    ...
    public class ApplicationDbInitializer : DropCreateDatabaseIfModelChanges<ApplicationDbContext>
    {
        ...
        public static void InitializeIdentityForEF(ApplicationDbContext db)
        {
            ...
            const string roleName1 = "Employee";
            ...
            string userid = userManager.Users.Where(u => u.Email == "admin@example.com").FirstOrDefault().Id;
            var rolesForUser = userManager.GetRoles(userid);
            if (!rolesForUser.Contains(role.Name))
            {
                var result = userManager.AddToRole(user.Id, role.Name);
            }
            // Add user admin to Role Employee if not already added
